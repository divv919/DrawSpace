...
export default function Dialog({ children }: { children: React.ReactNode }) {
  return (
    <div className="font-manrope w-fit max-w-[300px] h-fit bg-neutral-800 rounded-md flex flex-col gap-2 py-4 px-5">
      {children}
    </div>
  );
}
...
export function DialogContent({ children }: { children: React.ReactNode }) {
  return (
    <div className="text-neutral-300 text-sm flex flex-col gap-2">
      {children}
    </div>
  );
}
export function DialogTitle({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex text-white text-lg font-semibold gap-2 items-center">
